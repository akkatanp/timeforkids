	...
	var imageDialog = function( editor, dialogType )
	{
		...
		return {
			...
			onShow : function()
			{
				...
				this.creditElement = false;
				
				// Default: create a new element.
				this.imageEditMode = false;
				this.linkEditMode = false;
				this.wrapperEditMode = false;
				...
				var editor = this.getParentEditor(),
					...
					wrapper = element.getAscendant( 'div' ),
				...
				if ( wrapper )
				{
					...
					this.wrapperEditMode = true;

					// Look for Image element.
					var wrapperChildren = wrapper.getChildren();
					for ( var i = 0; i < wrapperChildren.count(); i++ )			
					{
						var childTagName = wrapperChildren.getItem(i).getName();
						if ( childTagName == 'img' || childTagName == 'input' )
						{							
							this.imageElement = wrapperChildren.getItem(i);
							if ( this.imageElement.getName() == 'img' )
							{
								this.imageEditMode = 'img';
							} else if ( this.imageElement.getName() == 'input' ) {
							...
							if ( wrapperChildren.getItem( i ).getAttribute( 'class') == 'caption' ) 
							{
								this.captionElement = wrapperChildren.getItem(i);
							} else {
								this.creditElement = wrapperChildren.getItem(i);
							}
						}
					}
				}
			...
			onOk : function()
			{
				...
				// Set the span for the credit
                                if (this.imageElement.getAttribute( 'rel' )) {
